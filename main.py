...
SUITS = ["♠", "♥", "♦", "♣"]  # Change to "S", "H", "D", "C" if your terminal hates these.
...
def bj_print_hand(label, hand, hide_first=False):
    ...
    print(bj_join_cards(display_cards))
    if not hide_first:
        print(f"Total: {bj_hand_value(hand)}")
    print()
...
def bj_play_round(bet):
    ...
    player_total = bj_hand_value(player_hand)
    dealer_total = bj_hand_value(dealer_hand)

    bj_print_hand("Dealer's hand:", dealer_hand, hide_first=True)
    bj_print_hand("Your hand:", player_hand)
    ...
    if player_total == 21:
        ...
        print(f"Blackjack! You win {winnings} money (3:2 payout).\n")
        return winnings

    # Dealer Blackjack only
    if dealer_total == 21:
        bj_print_hand("Dealer's hand:", dealer_hand, hide_first=False)
    ...
    while True:
        ...
        if choice == "h":
            ...
            player_total = bj_hand_value(player_hand)
            if player_total > 21:
                print(f"You bust! Dealer wins. You lose {bet}.\n")
                return -bet
        else:
            break
    ...
    if dealer_total > 21:
        ...
        print("It's a push (tie). No money won or lost.\n")
        return 0.0


def open_blackjack_minigame():
    """
    Pause the idle game UI, play Blackjack using game['money'] as bankroll,
    then return and update game['money'].
    """
    global listener_enabled, last_render

    listener_enabled = False   # stop key_listener grabbing stdin
    clear_screen()

    bj_print_title()
    print("Welcome to ASCII Blackjack with betting!")
    print("You will use your current money as chips.")
    print("Blackjack pays 3:2.\n")
    print("Press 'b' at the bet prompt to return to your desk.\n")

    bankroll = float(game.get("money", 0.0))
    ...
    print(f"You walk into the casino with ${format_number(bankroll)}.\n")

    try:
        while True:
            if bankroll <= 0:
                print("\nYou are out of money. The house wins this time.")
                break

            print(f"Current money: {bankroll:.2f}")
            # Get bet
            while True:
                bet_str = input(
                    f"Place your bet (1 - {int(bankroll)}) or 'b' to go back: "
                ).strip().lower()

                if bet_str == "b":
                    # Cash out and return to main game
                    print(f"\nYou cash out with {bankroll:.2f}.")
                    game["money"] = bankroll
                    save_game()
                    input("Press Enter to return to your desk...")
                    listener_enabled = True
                    last_render = ""
                    clear_screen()
                    return
            ...
            net_change = bj_play_round(bet)
            bankroll += net_change
            print(f"Change this round: {net_change:+.2f}")
            print(f"New total: {bankroll:.2f}\n")
            ...
            clear_screen()
            bj_print_title()
            print(f"You still have {bankroll:.2f}.\n")

    finally:
        # Update game money and return to main UI
        game["money"] = bankroll
        save_game()
        print(f"\nYou leave the table with {bankroll:.2f}.")
        input("Press Enter to return to your desk...")
...
def render_ui(screen="work"):
    ...
    middle_lines += ["", "Options: [W]ork  [U]pgrades  [B]lackjack  [Q]uit"]
        ...
        while running:
            ...
            if KEY_PRESSED:
                ...
                if k == "q":
                    ...
                    open_upgrade_menu()
                    render_ui(screen=current_screen)
                elif k == "b" and current_screen == "work":
                    # Open Blackjack minigame, using money as chips
                    open_blackjack_minigame()
                    last_tick_time = time.time()
                    current_screen = "work"
                elif k == "f":
